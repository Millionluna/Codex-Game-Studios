...
import { generateProfileRewriteDraft } from "@/lib/openai-profile-rewrite";
import {
...
export async function POST(request: Request) {
  let body: ProfileRewritePostBody;

  try {
    body = (await request.json()) as ProfileRewritePostBody;
  } catch {
  ...
  const model =
    process.env.OPENAI_PROFILE_REWRITE_MODEL?.trim() ||
    process.env.OPENAI_MODEL?.trim() ||
    DEFAULT_PROFILE_REWRITE_MODEL;

  try {
    const generated = await generateProfileRewriteDraft({
    ...
    if (message.includes("quota exhausted")) {
      return NextResponse.json(
        { ok: false, error: "Daily guided AI quota exhausted" },
        { status: 429 },
      );
    }
    ...
    if (message.includes("Access code required")) {
      return NextResponse.json(
        { ok: false, error: "Access code required for guided AI usage" },
        { status: 403 },
      );
    }

    return NextResponse.json(
      { ok: false, error: "Unable to generate profile rewrite draft" },
      { status: 502 },
    );
  }
}
