...
import { generateShareCardDraft } from "@/lib/openai-share-card";
import {
...
export async function POST(request: Request) {
  let body: ShareCardPostBody;

  try {
    body = (await request.json()) as ShareCardPostBody;
  } catch {
  ...
  const model =
    process.env.OPENAI_SHARE_CARD_MODEL?.trim() ||
    process.env.OPENAI_MODEL?.trim() ||
    DEFAULT_SHARE_CARD_MODEL;

  try {
    const generated = await generateShareCardDraft({
    ...
    return NextResponse.json(
      { ok: false, error: "Unable to generate share card draft" },
      { status: 502 },
    );
  }
}
