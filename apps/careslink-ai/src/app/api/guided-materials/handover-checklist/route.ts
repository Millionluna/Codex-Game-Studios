...
import { generateHandoverChecklistDraft } from "@/lib/openai-handover-checklist";
import {
...
type HandoverChecklistPostBody = {
  draftId?: unknown;
};
...
export async function POST(request: Request) {
  let body: HandoverChecklistPostBody;

  try {
    body = (await request.json()) as HandoverChecklistPostBody;
  } catch {
  ...
  const model =
    process.env.OPENAI_HANDOVER_CHECKLIST_MODEL?.trim() ||
    process.env.OPENAI_MODEL?.trim() ||
    DEFAULT_HANDOVER_CHECKLIST_MODEL;

  try {
    const generated = await generateHandoverChecklistDraft({
    ...
    return NextResponse.json(
      { ok: false, error: "Unable to generate handover checklist draft" },
      { status: 502 },
    );
  }
}
