...
type ReferralMessageApiResponse =
  | {
      ok: true;
...
export type GuidedReferralMessageDisabledReason =
  | "verified_session_required"
  | "claimed_draft_required"
  | "access_required";

type GuidedReferralMessageGeneratorProps = {
  draftId?: string;
  profileName: string;
  enabled: boolean;
  disabledReason?: GuidedReferralMessageDisabledReason;
  copy: ReferralWorkspaceCopy["materials"]["referralMessageGenerator"];
};

type RequestState = "idle" | "loading" | "success" | "error";

export function GuidedReferralMessageGenerator({
  draftId,
  profileName,
  enabled,
  disabledReason = "access_required",
  copy,
}: GuidedReferralMessageGeneratorProps) {
  const [requestState, setRequestState] = useState<RequestState>("idle");
  const [material, setMaterial] = useState<
    ReferralMessageMaterial | undefined
  >();
  const [errorMessage, setErrorMessage] = useState("");
  const isLoading = requestState === "loading";
  const canSubmit = enabled && !isLoading;

  async function generateReferralMessageDraft() {
    if (!enabled) {
      return;
    }
    ...
    try {
      const response = await fetch("/api/guided-materials/referral-message", {
        method: "POST",
        headers: {
      ...
      const body = (await response.json()) as ReferralMessageApiResponse;

      if (!response.ok || !body.ok) {
        setMaterial(undefined);
        setRequestState("error");
        setErrorMessage(
          getGuidedReferralMessageErrorMessage(response.status, body, copy),
        );
        return;
      }
  ...
  return (
    <div data-testid="guided-referral-message-generator">
      <Card className="p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <button
            type="button"
            disabled={!canSubmit}
            onClick={generateReferralMessageDraft}
          ...
          <div className="mt-5">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <ReferralMessageField
                label={copy.fieldLabels.subjectLine}
                value={material.subjectLine}
              />
              <ReferralMessageField
                label={copy.fieldLabels.opening}
                value={material.opening}
              />
              <ReferralMessageField
                label={copy.fieldLabels.providerSummary}
                value={material.providerSummary}
              />
              <ReferralMessageField
              ...
              />
              <ReferralMessageField
                label={copy.fieldLabels.handoverRequest}
                value={material.handoverRequest}
              />
              <ReferralMessageField
...
function getGuidedReferralMessageErrorMessage(
  status: number,
  body: ReferralMessageApiResponse,
  copy: ReferralWorkspaceCopy["materials"]["referralMessageGenerator"],
) {
