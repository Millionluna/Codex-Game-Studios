...
type BilingualIntroApiResponse =
  | {
      ok: true;
...
export type GuidedBilingualIntroDisabledReason =
  | "verified_session_required"
  | "claimed_draft_required"
  | "access_required";

type GuidedBilingualIntroGeneratorProps = {
  draftId?: string;
  profileName: string;
  enabled: boolean;
  disabledReason?: GuidedBilingualIntroDisabledReason;
  copy: ReferralWorkspaceCopy["materials"]["bilingualIntroGenerator"];
};

type RequestState = "idle" | "loading" | "success" | "error";

export function GuidedBilingualIntroGenerator({
  draftId,
  profileName,
  enabled,
  disabledReason = "access_required",
  copy,
}: GuidedBilingualIntroGeneratorProps) {
  const [requestState, setRequestState] = useState<RequestState>("idle");
  const [material, setMaterial] = useState<BilingualIntroMaterial | undefined>();
  const [errorMessage, setErrorMessage] = useState("");
  const isLoading = requestState === "loading";
  const canSubmit = enabled && !isLoading;

  async function generateBilingualIntroDraft() {
    if (!enabled) {
      return;
    }
    ...
    try {
      const response = await fetch("/api/guided-materials/bilingual-intro", {
        method: "POST",
        headers: {
      ...
      const body = (await response.json()) as BilingualIntroApiResponse;

      if (!response.ok || !body.ok) {
        setMaterial(undefined);
        setRequestState("error");
        setErrorMessage(
          getGuidedBilingualIntroErrorMessage(response.status, body, copy),
        );
        return;
      }
  ...
  return (
    <div data-testid="guided-bilingual-intro-generator">
      <Card className="p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <button
            type="button"
            disabled={!canSubmit}
            onClick={generateBilingualIntroDraft}
        ...
        {material ? (
          <div className="mt-5">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <BilingualIntroField
                label={copy.fieldLabels.englishIntro}
                value={material.englishIntro}
              />
              <BilingualIntroField
                label={copy.fieldLabels.communityLanguageIntro}
                value={material.communityLanguageIntro}
              />
              <BilingualIntroField
                label={copy.fieldLabels.language}
                value={material.language}
              />
              <BilingualIntroField
                label={copy.fieldLabels.sharingContext}
                value={material.sharingContext}
              />
...
function getGuidedBilingualIntroErrorMessage(
  status: number,
  body: BilingualIntroApiResponse,
  copy: ReferralWorkspaceCopy["materials"]["bilingualIntroGenerator"],
) {
