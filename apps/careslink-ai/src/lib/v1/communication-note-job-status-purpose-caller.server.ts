...
import {
  ...
  createCaresLinkV1CommunicationNoteJobStatusRepository,
  type CaresLinkV1CommunicationNoteJobStatusRepository,
...
const RUNTIME_ROLE_PATTERN =
  /^careslink_v1_job_status_runtime_[a-f0-9]{16}$/;
...
export const CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_PURPOSE_CALLER_VERSION =
  "communication.job-status.synthetic-preview.2026-09-07.v1" as const;
export const CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_PURPOSE_CALLER_READY =
  false as const;
export const CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_DATABASE_PURPOSE =
  "COMMUNICATION_NOTE_JOB_STATUS_READ" as const;
export const CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_CALLER_ROLE =
  "careslink_v1_generation_job_status_caller" as const;

export { CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_POSTGRES_SQL };
...
const POLICY_CORE = deepFreeze({
  ...
  callerRole: CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_CALLER_ROLE,
...
export const CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_PURPOSE_CALLER_POLICY_DIGEST =
  "a873781414a0e6e7b612ffb7944bae2add3297d301bcfe1c54aee87a11588d72" as const;
...
export const CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_PURPOSE_CALLER_POLICY =
  deepFreeze({
    ...POLICY_CORE,
    policyDigest:
      CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_PURPOSE_CALLER_POLICY_DIGEST,
...
export type CaresLinkV1CommunicationNoteJobStatusCredentialResolver =
  Readonly<{
    acquire(
      request: unknown,
      context: BoundedCallContext,
...
export type CaresLinkV1CommunicationNoteJobStatusPreviewDatabaseTarget =
  Readonly<{
...
export type CaresLinkV1CommunicationNoteJobStatusPurposeCallerAdapter =
  Readonly<{
    status: "SOURCE_ADAPTER_NOT_ACTIVATED";
    purpose: typeof CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_DATABASE_PURPOSE;
    callerRole: typeof CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_CALLER_ROLE;
...
export const CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_PURPOSE_CALLER_ADAPTER =
  undefined as
    | CaresLinkV1CommunicationNoteJobStatusPurposeCallerAdapter
    | undefined;
 ...
 */
export function createCaresLinkV1CommunicationNoteJobStatusPreviewDatabaseTarget(
  value: unknown,
): CaresLinkV1CommunicationNoteJobStatusPreviewDatabaseTarget {
...
/**
 * Captures an injected purpose credential resolver. The runner-terminal or Points admission
 * resolver is deliberately incompatible and cannot satisfy this brand.
 */
export function createCaresLinkV1CommunicationNoteJobStatusCredentialResolver(
  value: unknown,
): CaresLinkV1CommunicationNoteJobStatusCredentialResolver {
  const object = exactDataRecord(value, ["capability", "acquire", "revoke"]);
  if (
    object.capability !== "INJECTED_JOB_STATUS_CREDENTIAL_RESOLVER" ||
  ...
  const resolver = Object.freeze({
    acquire:
      object.acquire as CaresLinkV1CommunicationNoteJobStatusCredentialResolver["acquire"],
    revoke:
      object.revoke as CaresLinkV1CommunicationNoteJobStatusCredentialResolver["revoke"],
  });
 ...
 */
export function createCaresLinkV1CommunicationNoteJobStatusPurposeSessionLease(
  value: unknown,
  ...
  if (
    object.capability !== "INJECTED_JOB_STATUS_EXCLUSIVE_SESSION" ||
 ...
 * no connection itself. Each repository accepts exactly one get call and
 * obtains exactly one short-lived, purpose-bound physical session for it.
 */
export function createCaresLinkV1CommunicationNoteJobStatusPurposeCallerAdapter(
  value: unknown,
): CaresLinkV1CommunicationNoteJobStatusPurposeCallerAdapter {
...
async function executeOneStatusRead(input: Readonly<{
  ...
  requireNotAborted(input.signal);
  validateDatabaseTargetFreshness(input.databaseTarget, readClock(input.clock));
  return result;
}

function createAcquisitionRequest(
  target: CaresLinkV1CommunicationNoteJobStatusPreviewDatabaseTarget,
  observedAt: string,
) {
  ...
  const core = deepFreeze({
    ...
    acquisitionNonceSha256: canonicalSha256(randomUUID()),
    observedAt,
    requestedExpiresAt,
...
function validateSessionLeaseDescriptorShape(value: unknown) {
  ...
  if (
    ...
    rpcNames[0] !== CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_RPC_NAME ||
    object.rpcParameterCount !== 5 ||
...
function validateRepositoryCall(sql: string, values: readonly unknown[]) {
  if (
    sql !== CARESLINK_V1_COMMUNICATION_NOTE_JOB_STATUS_POSTGRES_SQL ||
...
function requireDatabaseTarget(value: unknown) {
  ...
  return value as CaresLinkV1CommunicationNoteJobStatusPreviewDatabaseTarget;
}
...
function requireCredentialResolver(value: unknown) {
  ...
  return value as CaresLinkV1CommunicationNoteJobStatusCredentialResolver;
}
...
function normalizeRowsOnly(value: unknown) {
  ...
  if (!Array.isArray(object.rows) || nodeTypes.isProxy(object.rows) ||
      Reflect.ownKeys(object.rows).length !== 2) {
    throw unavailable();
  }
  const rows = exactDataArray(object.rows, 1);
  return Object.freeze({ rows: Object.freeze([Object.getOwnPropertyDescriptor(rows, "0")!.value]) });
}
...
function unavailable() {
  return new CaresLinkV1ContractError(
    ...
    "Communication Note job status purpose caller is unavailable",
  );
}
