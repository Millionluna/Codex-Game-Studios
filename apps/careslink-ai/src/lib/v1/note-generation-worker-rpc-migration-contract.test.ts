...
describe("V1 Note registered-worker RPC shadow migration contract", () => {
  ...
  it("ships rollback-only catalog, ACL, arbitration and atomicity assertions", () => {
    ...
    expect(assertionHeader).toContain("this file remains serial evidence");
    for (const marker of [
    ...
    for (const marker of [
      ...
      "PostgreSQL 17.6 r22",
      "15/15 migrations, 7/7 rollback suites",
      "12-table/9-RPC/hard-off/zero-fixture/API-denial",
      "validated retention-FK/index postcheck all passed",
      "US$0.01344/hour",
      "v1-note-worker-rpc-r22",
      "0bc8db56-0e4a-42ec-9595-1f32a3d74a6b",
      "wuzcjcfrkctelcnbbgtg",
      "persistent=false, with_data=false",
      "23 INFO + 3 pre-existing WARN",
      "20 INFO (14 unindexed FKs + 6 unused indexes)",
