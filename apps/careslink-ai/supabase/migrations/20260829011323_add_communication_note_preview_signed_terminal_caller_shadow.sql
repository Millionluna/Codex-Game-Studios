-- Communication Note Preview independently signed runner-terminal ingress.
--
-- Additive, source-only and default-off. The new caller is a purpose-scoped
-- NOLOGIN shell. It receives no credential, membership, row privilege, HTTPS
-- path or runtime adapter here. An application trust registry must verify the
-- Ed25519 signature and terminal-only key purpose before entering the caller.
...
-- This hard cut-over changes the signed statement schema and adds NOT NULL
-- evidence columns. Serialize every writer and refuse implicit backfill.
...
alter table careslink_v1_generation.communication_note_preview_runner_terminals
  ...
  add column signature_sha256 text not null unique,
  add column signer_key_id_hash text not null,
  add column signer_public_key_sha256 text not null,
  add column authenticity text not null,
  add column verifier_method text not null;

alter table careslink_v1_generation.communication_note_preview_runner_terminals
  drop constraint comm_preview_runner_terminals_policy_check,
  add constraint comm_preview_runner_terminals_policy_check check (
  ...
  add constraint comm_preview_runner_terminals_signed_envelope_check check (
    ...
    and authenticity = 'EXTERNAL_RUNNER_TERMINAL_ED25519_VERIFIED'
    and verifier_method = 'APPLICATION_ED25519_TERMINAL_TRUST_REGISTRY'
  );
...
-- Prove the fifth caller has no path to any earlier purpose-specific RPC.
...
-- Do not leave an unsigned overload that a future ACL could expose.
drop function
