...
| `CARESLINK_COMMUNICATION_NOTE_GENERATION_API_ENABLED` | Reserved server configuration | independent M1x gate for `POST /api/ai-documents/communication-note/generate` | exact `true` is still insufficient: compile-time readiness is `false` and the formal submitter is absent, so the route returns `503` before auth/body access; do not configure as an activation step |
