...
| `CARESLINK_V1_POINTS_UI_ENABLED` | Server configuration | mutually exclusive server-rendered cutover from the legacy Credits page to the read-only Points preview on `/plan-and-usage` | only exact `true` selects the Points surface; unset, `false` or any other value keeps the legacy page. In Points mode the page never reads or renders the legacy credit store, and Points failures never fall back to Credits. This flag grants no API or database authority and must remain off until the same Preview cohort can no longer reach legacy Credits balances on the AI Documents and NDIS Companion surfaces |
