...
| `CARESLINK_V1_POINTS_UI_ENABLED` | Server configuration | mutually exclusive Points presentation and legacy NDIS kill switch | only exact `true` selects the read-only Points surface on `/plan-and-usage`, removes legacy Credits reads and generation links from `/ai-documents`, requires the flag for the Communication Note Points preview, replaces the direct NDIS Companion with a server-rendered unavailable state, and makes its legacy generation and telemetry `POST` routes fail closed before body, store, rate-limit or model work. Unset, `false` or any other value preserves the legacy experience. Points failures never fall back to Credits. This flag grants no Product API, database, Point mutation or model authority and remains `false` in the example. Activation requires one coordinated Preview cutover and a check for in-flight legacy reservations. |
