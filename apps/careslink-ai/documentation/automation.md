...
- Portal Referral intake, source detail and Assignment M1a are synchronous request/transaction paths, not automation. They use a request-scoped cookie Supabase client and eleven restricted public database RPCs in total—three intake, two source-detail and six assignment. The assignment "queue" is an operator read model, not a background job queue; these paths use no `service_role`, OpenAI call, worker, cron, notification or unattended retry.
...
| Portal API + durable-adapter + independent operation gate | base gates exact `true`, `VERCEL_ENV=preview`, an exact non-Production Supabase ref, the selected intake/detail/assignment application gate, the master database row and its matching operation row are required | any failed gate stops before a client or private body read where applicable; intake, source detail and assignment do not enable one another, including through direct Data API calls; exact-current Assignment has deleted-Preview Hosted Cookie evidence, but no runtime is retained or deployed and every gate is disabled |
