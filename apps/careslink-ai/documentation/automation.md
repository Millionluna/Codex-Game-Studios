...
| Portal API + durable-adapter + independent operation gate | base gates exact `true`, `VERCEL_ENV=preview`, an exact non-Production Supabase ref, the selected intake/detail/assignment/provider-response application gate, the master database row and its matching operation row are required | any failed gate stops before a client or private body read where applicable; the four operation slices do not enable one another, including through direct Data API calls. Exact-current Assignment retains its deleted-Preview Hosted Cookie evidence. Historical M1b source `cc1e53cc88666a3e3f18ac55058295db408535ee` separately passed 33/33 migrations, 14/14 rollback suites and the real GoTrue SSR-cookie/Data API matrix on deleted no-data Preview ref `aupndcptwlqmjlgeifdj`; teardown proved all Auth/Portal fixture domains at zero, three deletion probes passed and Production remained at 19 migrations. Post-review source `f45b19c596edd0bdbe01eba17e6e5fa136df5225` adds active-offer-first selection, request timeouts, principal-boundary command clearing and 6/6 local true-concurrency scenarios, but the deleted Hosted run does not cover it. M1b remains default-off and Production-unapplied: no runtime is retained or deployed, every gate is disabled, and a new authorized no-data Hosted re-gate is required before merge/promotion; no activation, private-detail/follow-up/audit/export automation or Vercel/Production deployment was added |
