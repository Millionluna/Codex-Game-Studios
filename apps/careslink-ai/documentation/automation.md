...
- Portal Referral intake and source detail are synchronous request/transaction paths, not automation. They use a request-scoped cookie Supabase client and five restricted public database RPCs in total—three intake and two source-detail; they use no `service_role`, OpenAI call, worker, queue, cron, notification or unattended retry.
...
| Portal API + durable-adapter + independent operation gate | base gates exact `true`, `VERCEL_ENV=preview`, an exact non-Production Supabase ref, the selected intake/detail application gate, the master database row and its operation row are required | any failed gate stops before a client or private body read where applicable; list/create and source detail do not enable each other, including through direct Data API calls; source/local only, currently unhosted and disabled |
