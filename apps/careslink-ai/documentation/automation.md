...
- Portal Referral intake is a synchronous request/transaction path, not automation. It uses a request-scoped cookie Supabase client and three restricted database RPCs; it uses no `service_role`, OpenAI call, worker, queue, cron, notification or unattended retry.
...
| Portal API + durable-adapter + intake gates | all three exact `true`, `VERCEL_ENV=preview`, an exact non-Production Supabase ref and the separate database flag are required for list/create | any failed gate stops before a client or private body read where applicable; source/local only, currently unhosted and disabled |
