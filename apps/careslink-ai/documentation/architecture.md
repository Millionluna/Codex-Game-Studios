...
> Status: **Current-state architecture**, audited 2026-08-16; source-only
...
- The current Communication Note source conditionally installs its formal
  Cookie principal only when the complete exact non-Production Preview,
  project, URL, publishable-key and application-gate binding passes. Its UI and
  API compile-time readiness latches remain `false`; the formal submitter and
  encrypted-payload maintenance compositions remain `undefined`, so no facts
  can reach database admission or a model through this path.
...
| Portal Referral intake, source-detail, Assignment M1a and Provider Response M1b runtimes | `portal-referral-runtime.server.ts`, `portal-referral-supabase.server.ts`, the Portal route adapter, dedicated assignment/provider controls and four operation migrations implement request-scoped cookie authorization, source list/create/detail, operator queue/detail/triage/candidates/offer and provider own-offer list/accept/decline | default-off source implementation with separate application and database operation gates. Provider Response adds `GET /api/portal/referral-offers`, `POST /api/portal/referral-offers/{matchId}/response` and three authenticated-only definer RPCs. Exact-one approved-provider authority is database-derived; its one-page no-PII inbox selects live offers before accepted history; accept/decline are actor+mutation serialized and receipt/audit hashed. Gate-on pages never fall back to mock; same-epoch network uncertainty retains the mutation key, while browser lifecycle reauthorization clears the old projection and command so a different same-provider member cannot inherit it. All five database flags default off/Preview-only. Post-review implementation source `f45b19c596edd0bdbe01eba17e6e5fa136df5225` passed 8/271 focused and 143/1,935 full tests, the PostgreSQL 16.15 7/7 migration + 4/4 rollback gate and 6/6 true two-backend races. Exact gate source HEAD `44f3bd68699dc953e2666bf033dac2b5e26a4d30` then passed the disposable no-data Hosted re-gate on deleted ref `nhupgyxczlvtddycrgyw`: 33/33 migrations, 14/14 rollback suites, the real two-provider GoTrue SSR-cookie/Data API matrix 14/14, and active-first/non-null-cursor delta 2/2. Private accepted detail, follow-up, notifications, audit list, document/export, deployment and activation remain absent. No retained Preview/runtime, Vercel deployment, merge, Production migration/write or activation exists; teardown left zero Auth and Portal fixture rows while retaining the 33 migration records and five disabled Preview-only flags |
| Portal Referral Follow-up M1c runtime | `portal-referral-runtime.server.ts`, `portal-referral-supabase.server.ts`, the dedicated provider follow-up coordinator and CLI-generated `20260826090841_add_portal_referral_follow_up_runtime.sql` implement exact accepted-provider private detail plus fixed-code follow-up | independently default-off and Production-unapplied. Three authenticated-only definer RPCs re-derive exact approved-provider authority, bind detail to assigned provider plus coherent accepted match, and atomically write referral version, append-only follow-up, metadata-only audit and hash-only receipt. Receipt replay uses the fresh mutation's referral→ordered-match lock order and revalidates session/live authority after locking. The client clears private state on identity/resource changes and preserves no PII in uncertain replay state. Source/local tests and a retained private-Unix-socket PostgreSQL 16.15 lifecycle prove 8/8 true multi-backend replay/competition/revocation/ownership races twice with exact cleanup and an exact-hash-pinned live harness. No Hosted GoTrue/PostgREST, retained Preview, deployment, activation or Production application exists |
...
| Communication provider-evaluation M1d | `communication-note-provider-policy.ts`, `communication-note-fact-parity.ts`, `communication-note-golden.ts` and `openai-communication-note-provider.server.ts` introduced a digest-bound evaluation policy, deterministic date/time/Arabic-quantity parity, three synthetic multilingual golden/refusal fixtures and a Responses request/parser contract with strict structured output, bounded JSON and content-free errors/evidence | historical source-only checkpoint now superseded by M1e's exact model/data plan and fail-closed transport boundary; full semantic groundedness remains blocked; zero live calls; no key/env lookup, route, payload vault, worker binding, Points, UI, deployment or Production activation; readiness remains `false` |
| Communication Preview-evaluation policy M1e | `communication-note-preview-evaluation-policy.ts` added literal-pinned fixture and canonical plan digests and bound the request builder to the exact provider-policy digest, immutable `gpt-5.4-mini-2026-03-17` model ID, closed AU-storage Responses endpoint profile and explicit `reasoning:none`; it recorded synthetic-only, no-retry, six-call/US$0.25 ceilings and all-pass automated plus human-semantic requirements | historical source-policy checkpoint, now superseded by M1f's request-template, manifest and runner bindings; it authorized no provider call |
| Communication one-shot Preview-evaluation runner M1f | `communication-note-openai-request-template.ts`, `communication-note-preview-evaluation-manifest.ts` and `communication-note-preview-evaluation-runner.server.ts` literal-pinned the static request semantics, exact six-slot order, no-retry worker policy and one-shot runner. The runner preflighted every positive input count before dispatch, hashed each source-rendered request object, reserved a 120,780-micro-USD maximum below the US$0.25 cap, ran serially, reconciled strict provider usage, required all seven checks plus 18 language reviews and returned a content-free, explicitly unattested self-digested report | historical source-only mock contract now extended by M1g-a's request-body pins. M1f itself did not independently pin exact JSON body bytes. Paid readiness remained `false`, the approved runner policy was `undefined`, the paid factory failed closed and the executable factory accepted a branded test provider plus arbitrary trusted test callbacks that were not a security boundary; zero real calls |
| Communication request-body pins M1g-a | `communication-note-openai-request-wire.ts` and `communication-note-preview-request-body-pin.ts` bind wire version `wire.communication.openai.responses.2026-08-27.m1g-a.v1`, body-pin version `pin.communication.openai.synthetic-request-body.2026-08-27.m1g-a.v1`, bundle digest `90b9c42796f5d649fcadcdc0cb4c7f123f4d20c79d3c74f2e27e79fe6ec802e8` and six ordered slots comprising three distinct literal request bodies. Each slot carries raw JSON-body SHA-256, UTF-8 byte length and semantic canonical digest. Runner version `runner.communication.openai.synthetic-preview.2026-08-27.m1g-a.v2` with digest `a604057aceed70b741d4e1ac2a0e1f9bdf5d13721955448ec083948fb8b4a7c4` validates the source pin before preflight; the provider sends the same validated `JSON.stringify` string without reserialization through the non-HTTPS mock callback; the report validator compares literal pins without rebuilding requests | source-review evidence only: scope is the JSON request body and a source-declared application-envelope policy, not a mock-dispatch attestation or full HTTP/TLS transport capture. Authenticity is `UNATTESTED_SOURCE_PIN_ONLY`, execution authority is `NOT_EXECUTION_AUTHORITY`, external owner approval and dispatch attestation are absent, body-pin and paid-runner readiness are `false`, and the paid factory remains unavailable. No key, real network call, authenticated receipt, product runtime importer, route, worker, Points, deployment or Production activation |
| Communication owner authorization and dispatch receipt M1g-b | `communication-note-preview-execution-authority.server.ts` defines an external-trust-registry Ed25519 authorization envelope, exact six-slot/source/environment/budget bindings and CaresLink-signed content-free dispatch observations. Authority policy digest `7804c7d60bb8c686d66a4c0aed74b373023dda672f1ebfa0a8e7c8af4eb7a9d9` binds a 15-minute maximum authorization, 5-minute minimum remaining claim window, atomic single-use claim, pre-network per-slot reservation, serial order, no retry and permanent consumption of ambiguous transport. The paired source migration defines private forced-RLS append-only ledgers and separate authorization, dispatch and receipt executor roles | source-only shadow contract. All five RPCs require `READ COMMITTED`; both readiness latches are literal `false`; approved owner and receipt keys, trust registry, caller grants, real transport and runtime importers are absent. PostgreSQL persists statements only after application-side Ed25519 trust-registry verification; it does not independently authenticate the owner. Receipts are `CARESLINK_SIGNED_INTERNAL_OBSERVATION` with `providerAttestation=ABSENT`, pairwise-distinct HMACs for the client request ID, OpenAI `x-request-id` and Responses `response.id`, and explicit non-proof of provider receipt, billing, model execution or exactly-once delivery. No real key/call/spend, hosted database mutation, Preview retention, route, worker, Points, deployment or Production activation |
| Communication key custody and exact callers M1g-c | `communication-note-preview-key-custody.server.ts` binds version `custody.communication.openai.synthetic-preview.2026-08-28.m1g-c.v1`, policy digest `1f7a3c586155fb4246e40207136cc1e521daedf6f2d01d1f89f7beebfad66438` and three purpose-separated, content-free injected descriptors: candidate metadata labelled as an external owner-verification snapshot, a non-exportable CaresLink receipt-signer custody-reference digest and a temporary OpenAI project service-account credential-reference digest. Migration `20260828034704_add_communication_note_preview_custody_callers_shadow.sql` adds four `NOLOGIN` caller shells with an exact 1/1/2/1 mapping to the five existing registration, revocation, claim/reservation and receipt RPCs; it adds no custody table or credential row | source-only and default-off. The approved snapshot is `undefined`, both M1g-c readiness latches are literal `false`, and the validator is `TEST_ONLY`. Caller shells get only private-schema usage plus exact function execution and have no login, usable `INHERIT`/`SET`, caller-to-executor/runtime, table, API or service-role privilege; PostgreSQL 16 may retain only its non-usable creator ADMIN bootstrap edge. Candidate registry hashes are not authenticated registry provenance. Caller-supplied identity HMACs are correlation/scope bindings, not database authentication. No real key, signing operation, service account, credential resolution, login-capable caller, runtime importer, OpenAI call, hosted mutation, route, worker, deployment or Production activation |
| Communication activation preflight M1g-d | `communication-note-preview-activation-preflight.server.ts` now binds version `preflight.communication.openai.synthetic-preview.2026-08-29.m1g-d.v2` and policy digest `791a4d893afd4e490ab0164a8f604589bcf8015d25e5723b4df210f8c0b44f67`. Its pure `TEST_ONLY` validator cross-binds M1g-b authorization, M1g-c custody, the 37-file migration manifest and six exact M1g-b/M1g-c/M1g-f artifact pins. It still requires exactly four ordered 1/1/2/1 caller candidates and explicitly records that the M1g-f terminal executor has no fifth caller or runtime execute authority | source-only activation-preflight evidence, never activation authority. Valid input still yields five literal blockers and `activationReady=false`; the live factory always throws and prior readiness/approval latches remain false/undefined. External evidence and database observations are injected consistency claims, not trusted control-plane/catalog/bytes attestations. No environment/network/SDK/resolver/database runtime, hosted action, paid call, deployment or Production change |
| Communication reserve-before-dispatch transcript M1g-e | `communication-note-preview-reserve-before-dispatch-coordinator.server.ts` now binds version `coordinator.communication.openai.synthetic-preview.2026-08-29.m1g-e.v2` and policy digest `4649f620bc60425d5ca40d308d167110befd4a29c772e9877ddbeac5eaaa3531`. Its pure `TEST_ONLY` validator retains the exact registration → preflight → claim → serial reservation → transport → signed receipt/persistence → runner acceptance/failure contract and now binds the M1g-f terminal policy plus source presence of a database-returned `reservedAt` and durable terminal ledger | source-only, default-off transcript validation, never an executable coordinator or database/provider attestation. The structures exist only in source; no trusted reserve result or terminal row is supplied, so `DATABASE_ATTESTED_RESERVED_AT_ABSENT` and `DURABLE_RUNNER_TERMINAL_STATE_ABSENT` remain fixed. Readiness/approval remains false/absent and the live factory throws. No callback, environment/network/SDK/database/key operation, hosted action, paid call, deployment or Production change |
| Communication durable runner terminal M1g-f | `communication-note-preview-runner-terminal-policy.server.ts` binds terminal policy version `policy.communication.openai.synthetic-preview.runner-terminal.2026-08-29.m1g-f.v1` and digest `4f38d9ea27e9673138350ecdbc294e14e200cd09247f07244433a51cb62f6f5a`. CLI migration `20260828235426_harden_communication_note_preview_reservation_runner_terminal_shadow.sql` adds a private forced-RLS append-only terminal ledger and isolated `NOLOGIN`/`NOINHERIT`/`NOBYPASSRLS` executor, returns the database-written UTC-millisecond `reservedAt` on fresh reserve and exact replay, and requires every earlier slot to have `COMPLETED` receipt plus immutable `ACCEPTED` runner terminal before continuation. `FAILED` is permanent, exact replay is write-free/no-authority, and the existing five ledgers must be locked and empty before migration | source-only and default-off. No fifth caller, login, runtime executor membership, API/service-role execute, credential, fixture, provider call, Hosted application, deployment or Production change is created. The terminal function is inaccessible to product runtime until a later custody/runtime batch atomically adds an independently scoped caller and evidence; readiness remains false |
| Communication signed runner-terminal caller and ports M1g-g | Historical M1g-g checkpoint: `communication-note-preview-runner-terminal-policy.server.ts` bound an independent Ed25519 terminal trust root with purpose `CARESLINK_RUNNER_TERMINAL`, domain `CARESLINK_COMMUNICATION_NOTE_PREVIEW_RUNNER_TERMINAL`, policy version `policy.communication.openai.synthetic-preview.runner-terminal.2026-08-29.m1g-g.v2` and digest `d0ac3b14ceb97535cfed935250566b59d8ac42a93123a750d3a686102a8d1cfa`. Custody version `custody.communication.openai.synthetic-preview.2026-08-29.m1g-g.v2` and digest `f537dc64e3c57a34b6db6d0d1c871c38a70bcb51c4d071e625b026f840a309ca` added a purpose-separated terminal signer descriptor and fifth `RUNNER_TERMINAL_PERSISTENCE` mapping. Its then-derived preflight/coordinator `m1g-g.v3` digests were `491481513a67198cba91babc3c172fc1f326f9ee7bdd883b3d1208c639bdaf73` and `f6609c2f357b5fda92ae5aa1b459dfb1e32b7893c3e8436e0e94a8ffa2bbe675`. CLI migration `20260829011323_add_communication_note_preview_signed_terminal_caller_shadow.sql` replaced the unsigned terminal RPC with `persist_verified_communication_note_preview_runner_terminal(jsonb,text,text)`, persisted signature/verifier evidence and added the exact fifth caller shell | historical source-only/default-off checkpoint. The fifth caller was `NOLOGIN`/`NOINHERIT`/`NOBYPASSRLS` with no table, broad function, executor-membership, API/service-role or login privilege. Its source ports created no connection or resolver and readiness remained false. The historical PostgreSQL 16.15 gate clean-applied 38/38; the then-current migration and rollback-assertion hashes were `b095785331c848d02cabc417eb3131fe2f9328564abef6fc0dd35bccd2980c5a` and `f8e8307718e3bdf0835b93cdac075279ae4f5ba3dbab287af46e1280ce587ad5`. These values describe that checkpoint, not the current explicit-transaction migration bytes. No Hosted action, provider/model call, deployment or Production change |
| Communication signed runner-terminal trust composition and disposable identity M1g-h | Historical M1g-h checkpoint: `communication-note-preview-runner-terminal-trust-composition.server.ts` branded one test-only custody/trust composition, while its disposable-Preview harness designed a ten-minute one-connection `NOINHERIT` LOGIN with SET-only fifth-caller membership, bounded secret transport and fixed cleanup | source/local PostgreSQL 16.15 passed that revision's 38 migrations and signed `FAILED`/`CANCELLED` evidence, but its first Hosted Preview stopped at `SCHEMA_ROLLBACK_ASSERTION_FAILED`; the identity/terminal gates for that M1g-h attempt were not run. That Preview was deleted. This historical failure does not contradict the later r20 M1g-i evidence, and neither result proves the current strict identity-policy `.2` envelope. Registry/composition statuses remain `TEST_ONLY_*_NOT_APPROVED`, readiness is false and no product activation occurred |
| Communication signed terminal ACCEPTED usage alignment M1g-i | Current M1g-i retains migration 39's exact nine-key signed usage and deterministic `C` collation while projecting only the three reconciliation labels for six-fact receipt comparison. Migrations 37/38 have explicit repository transactions and SHA-256 `09e69476de4b5b1b925a281f2943ef541e289aab6bef60ad92aace14d0c6d432` / `4c13bf50d7866a4b948475b598bb1c103fb625e59824be98c4e272c659da283f`; migration 39 is `3d2cc53df3cf17ea21a4f93aaf673f8e911fcc9a35b5309cf7c633c6802e448e`. Transactional policy `.6` binds manifest `60314eb32f7ac26027862e30b27e60460cf4d17d49061126f4366b08a0cbd3a2`; identity policy `.2` requires exact no-data/non-default/non-persistent healthy child and Production-parent cross-binding before CA or connection use. Preflight/coordinator v5 bind `0e2582040995753efe95baa071fee4e0b58fa105c79db8bfa673abd66e2d01a1` / `1f93fa2c0ba207a28cb706d922acc10bba8305f16c83c7973c70ae4d7ac7e5c2` | current source `02949d1a666fa8aa0496d3e995f1dd88c52a29a0` passed 172 files / 2,321 tests, TypeScript, zero-warning ESLint, 73-file adapter sync, 64/64 Webpack pages and isolated PostgreSQL 16.15 39/39. Deleted no-data PostgreSQL 17 Preview `careslink-note-terminal-m1g-i-v5-r2-20260830` (id `0e63cac9-d1dc-4096-9f65-c36de91c85fa`; ref `yrsgxbxislyenblphfdl`) then passed the pinned 19-row baseline, transactional 39/39, A01–A18, identity `.2`, signed `ACCEPTED`/replay/`IDEMPOTENCY_CONFLICT`, independent 39-row plus `[1,0,1,1,1,1]` and zero-role/session postcheck, and final Advisors with 0 ERROR. Three deletion probes left only default Production `ACTIVE_HEALTHY`. Exact-current Hosted evidence is closed through the pinned repository runner; native Supabase CLI migration apply, live custody/credential resolution, provider evaluation, human review and final approval remain absent. Readiness is false and no Production write/deployment occurred |
| Communication resolved custody/caller runtime binding M1j | Historical M1j TestOnly checkpoint: `communication-note-preview-runner-terminal-resolved-runtime-binding.server.ts` defined version `binding.communication.openai.synthetic-preview.2026-08-30.m1j.v1` and policy digest `6e33f6a6c061f539b75808afc27abfe6f33fedfcf28de5f7b2a5fdeed5faee04`. Its PostgreSQL 17 `SET LOCAL ROLE`/SET-only runtime model is superseded by M1l and is not the current binding. It remains useful only as source-only/default-off historical evidence; its TestOnly tombstone fields were resolver self-report, not durable broker evidence. No environment lookup, network/SDK client, raw credential/private key, product importer, migration, Preview, provider/model call, deployment or Production change was added |
| Communication durable caller-credential resolver M1k | `communication-note-preview-durable-caller-credential-resolver.server.ts` adapts M1j to injected broker, exclusive-session and independent-audit ports under version `resolver.communication.openai.synthetic-preview.2026-08-30.m1k.v1` and digest `67fb2065c23c1cd99a8e1c1a396509edefd7e7614fa476e2ee95839eafea5a7c`. The TypeScript resolver requests at most 90 seconds; the independently testable SQL broker accepts 30 seconds–10 minutes and the PG16 harness uses five-minute fixtures. The isolated `scripts/preview-e2e` broker serializes one acquisition digest with advisory transaction locks, persists a monotonic metadata-only state machine, atomically commits an irreversible tombstone plus OID/name-bound `NOLOGIN` fence before session destruction/finalization, creates a SCRAM runtime LOGIN with one SET-only caller edge, binds one physical backend and independently attests zero role/session/membership residue | TestOnly/local evidence only. Public construction always fails, readiness is `false`, the approved resolver is `undefined`, and no product importer, environment/SDK/network lookup, Supabase migration, Hosted action, provider/model call, deployment or Production change exists. The real disposable private-socket PostgreSQL 16.15 gate passed six lifecycle/concurrency groups, including a proven blocked acquire/tombstone race and rejected post-tombstone reconnect; postcheck observed three metadata tombstones plus zero runtime residue before teardown. It is not PostgreSQL 17/Hosted/TLS evidence. Production adoption still requires a separately authorized additive migration plus atomic repin, terminal-RPC ACTIVE-fence check, approved target/custody/secret transport and same-revision no-data PostgreSQL 17 Preview gate |
| Communication formal runtime-credential broker and terminal fence M1l | Binding `binding.communication.openai.synthetic-preview.2026-08-30.m1l.v1` / `cfb9f27b63f1a623950b3033fc04300149bcba26389994aa04eb2d2213ea1115` and durable resolver `resolver.communication.openai.synthetic-preview.2026-08-30.m1l.v2` / `e53114d9d247ffcdb20ed83b4724fa5b8b09eeab31e4f2fc1a868ade13a2f43e` bind formal additive migration source `20260830065750_add_communication_note_preview_runtime_credential_broker.sql`. Runtime is `INHERIT=true`; its outbound caller edge is `ADMIN=false`/`INHERIT=true`/`SET=false`, while its PostgreSQL 16/17 creator edge is inbound and inert. Client SQL sends no `SET ROLE`; the runtime inherits only the exact terminal wrapper. Cluster-wide `pg_shdepend`, exact wrapper/inner ACL and generation-column denial are revalidated at migration, acquire, bind and terminal time. The broker retains the shared terminal fence, exclusive tombstone/finalize fence and connection-bound cancellation quarantine | Production-unapplied/default-off. Local PostgreSQL 16.15 passed six scenarios and 4/4 issued/revoked acquisitions with zero residue. Deleted no-data/non-default/non-persistent PostgreSQL 17 Preview r5 (`5f088eac-ac66-4625-8f4c-c9e7d9b02c2a` / `ucdmoxqzruohiqmsokfv`) then passed the exact 40/40 single-transaction chain, A01–A18, pinned-CA terminal ACCEPTED/replay/conflict checks, direct inherited runtime without `SET ROLE`, bind-time `NOLOGIN`, exact large-object ACL/`pg_shdepend` `2BP01` rollback proof, controlled cleanup and independent zero-residue postcheck. Broker-scoped Security/Performance Advisor findings were zero, but global findings and pre-existing generation lint errors remain. r5 was deleted; three probes left only healthy default Production, which was never the SQL target. The 53/53 pin set remains frozen at migration `64dcb8c57f2c73d3fbd5adc99e3261f8e2e0ddd8e8efcf5cca52c12ca34ba5aa`, transactional manifest `6590eed19602c4d7931355f18dafde699b1c47012a3fe09f9d040c179e11792d` and rollback manifest `e0b5f30f9a4c33bf04020a4d11453c87a52321b69c6edd74982446b0fadd58fe`. Readiness remains `false`; approvals, Production migration, provider/model, product-driver, deployment and product-traffic evidence remain absent |
| Communication product runtime composition M1r | `communication-note-preview-product-runtime-composition.server.ts` binds version `composition.communication.openai.synthetic-preview.2026-09-01.m1r.v1`, policy digest `1227ff3dac4283749b62b8af953dea02d51da31f3edc0a9d4c3c62a9a1364af0` and the M1m policy digest. It statically imports and validates `pg.Client` from production dependency `pg@8.23.0`, keeps the constructor private, and injects the same constructor into M1m management/runtime slots so downstream sessions still instantiate separate clients | source-only/default-off. Formal construction always rejects with fixed `PRODUCT_API_DISABLED`, readiness is `false`, approved composition is `undefined`, and the only importer is its test. Cold import/formal construction perform no environment/DSN/credential/target discovery; the TestOnly path delegates only explicit injected resolver/transport ports to M1m. This batch invoked no real resolver, Client, SQL, Preview, Production migration, deployment or provider/model call. Target/control-plane/credential/deployment identities and all live activation approvals remain absent |
| Communication Note generation HTTP boundary M1x | `communication-note-generation-contract.ts`, `communication-note-generation-feature.ts`, `communication-note-generation-current-session.server.ts`, `communication-note-generation-principal.server.ts`, `communication-note-generation-principal-composition.server.ts`, `communication-note-generation-route.server.ts` and the physical Node route under `src/app/api/ai-documents/communication-note/generate` freeze a future Cookie mutation boundary plus a browser-safe transport DTO. The source-only strict-principal factory rejects every `Authorization` header before Auth work and uses one request-scoped Cookie/authenticated Supabase client in the fixed `getClaims` → frozen snapshot revalidation → zero-argument `resolve_v1_current_session_status()` → `getUser` order. The RPC receives no caller identity, and the resulting frozen `{userId, sessionId, transport: "COOKIE"}` principal is passed to admission. The wrapper keeps exact Vercel Preview/project, configured-ref/known-Production-denial, canonical-URL and matching-publishable-key guards. It reads no dedicated or generic privileged key, creates no privileged client, has no legacy two-argument RPC fallback and does not query Supabase branch provenance or status. Same-origin HTTPS, JSON byte limits, duplicate/unknown-key rejection, exact seven-field facts, two confirmations, dual privacy scans, idempotency and owner-safe job parsing remain enforced. This is source/local behavior, not live Cookie/Auth/database evidence | source-only/default-off. Compile-time readiness is `false`; the formal composition, principal resolver and submission port are all `undefined`. M1y now makes the route import only the two formal placeholders, without installing or calling either executable factory, so it still returns no-store `503` before request/auth access. The authenticated current-session migration remains unapplied. Formal installation, trusted Provider role normalization, same-transaction owner session/privacy reauthorization and a separately authorized no-data Hosted Preview active/revoked-session gate remain required before a registered asynchronous worker or any model-backed application work. The successor source-wires the UI and TestOnly enqueue path, but adds no model call, usable vault, worker invocation, Supabase/Preview/Production apply, deployment or activation |
| Communication Note product integration M1y | `communication-note-generation-client.ts`, `communication-note-generation-submitter-composition.server.ts`, the independent UI gate and the Communication Note composer now freeze a source path from locally reviewed facts through M1x, stable privacy proof, idempotent retention-bounded encrypted-payload staging and atomic fixed-20-Point admission ending at a durable attempt-zero `QUEUED` job. Strict projection prevents private admission fields from reaching the route; exceptions retain the stable bounded payload because commit outcome is unknown, while only a validated `payloadAccepted=false` response can request purge. The browser freezes one body/key pair, prevents duplicate submits, keeps at most one timer, caps automatic polls at 40, aborts a pending request on unmount and never decrements Points itself | source-only/default-off. UI and API compile-time latches are `false`, both formal compositions are `undefined`, and environment values cannot activate the path. The request thread invokes no worker/provider/model. Formal activation still requires policy-bound encrypted vault/KMS/backup custody, least-privilege database callers and live no-data Preview evidence, a payload consume path that can return a vault grant, an approved Communication-only or complete five-type worker registration, owner job/document recovery, provider budget/kill switch, deployment and separate activation approval. No real care data, Preview/Production write, model call or cloud mutation occurred |
| Communication Note secure submission runtime M1z | `note-generation-encrypted-payload-stager.server.ts` adds an injected-port AES-256-GCM envelope-encryption core with a fresh 32-byte DEK, 12-byte IV, canonical AAD, exact numeric KMS-version binding, conditional private-object creation, strict facts-hash/size checks and exact replay/conflict handling. `20260903041819_bind_v1_communication_note_encrypted_payload_admission.sql` adds a 19-argument private policy-bound Communication Note + fixed-20-Point coordinator and a separate `NOLOGIN` caller shell. `note-generation-encrypted-payload-maintenance.server.ts` adds content-free exact-admission reconciliation and expiry sweeping with exact-binding tombstone deletion | source-only/default-off. The formal Cookie principal is conditionally installed behind the complete exact Preview binding, but UI/API readiness remain `false`; the formal submitter, stager and maintenance exports remain absent. The migration is unapplied to every Supabase project, the caller has no credential/runtime member, and actual KMS, private object-store, query-credential and maintenance persistence adapters were missing at this checkpoint. No cloud resource, Preview/Production write, deployment, real care data or model call occurred. Focused tests passed 232/232; the complete local suite passed 3101/3101 across 219 files, with TypeScript, zero-warning ESLint, optimized build, 27-chunk client boundary, adapter sync and diff checks also passing |
| Communication Note secure submission provider adapters M2a | `note-generation-google-cloud-kms-wrap-adapter.server.ts` implements exact-numeric-version Cloud KMS `rawEncrypt` with AAD/CRC32C, a fresh branded exact-resource `RAW_ENCRYPT_DECRYPT` / `AES_256_GCM` / `SOFTWARE` / `ENABLED` posture and an exact synchronous direct-return credential handoff; `note-generation-encrypted-payload-gcs-private-object-store.server.ts` implements exact-bucket digest-only locators, generation-pinned direct `/download/storage/v1` reads, create-only writes and same-name CAS tombstones, while requiring injected claims that protection settings were effective for at least 30 seconds and historical noncurrent/soft-deleted versions are absent; `communication-note-points-admission-purpose-caller.server.ts` binds the existing exact 19-argument repository to one branded purpose-only physical session, requires at least 25 seconds of remaining lease/target validity, and validates termination/tombstone/quiescence receipts | source-only/default-off. Every formal export is `undefined` and every readiness latch is `false`. The posture brands, history flags and cleanup receipts validate injected claims but are not authenticated live trust. Credentials/transports, exact-resource control-plane provenance, authenticated post-change propagation/history/backup evidence remain absent; no WIF issuer, GCP key/bucket, Supabase login role, migration application, formal composition, route, scheduler or worker is installed. No network/cloud/Hosted/Production mutation, deployment, real care data or model call occurred. Focused tests passed 149/149; the complete local suite passed 3169/3169 across 222 files, with TypeScript, zero-warning ESLint, the 64/64-page optimized build, 27-chunk client boundary, adapter sync and diff checks also passing. See `documentation/communication-note-secure-submission-provider-adapters-m2a.md` |
...
| Communication Note Points read-only preview | `communication-note-points-preview.server.ts` and CLI-generated `20260902052755_add_v1_communication_note_points_preview.sql` bind the server-owned `note.communication.generate` rate to 20 Points and expose only the current authenticated Provider session's own shadow wallet `available`/`reserved` totals. The Communication Note page waits at most 1.5 seconds, parses an exact metadata DTO and otherwise fails closed to an unavailable state | source-only and explicitly labelled not active. The displayed shadow values are not a consumable real balance. The RPC and UI create no quote, reservation or welcome grant; do not debit or replace legacy credits; and cannot call a model, save a Note or export an artifact. The migration remains Production-unapplied and no Preview or Production deployment was made. The two successor source rows below couple reservation to admission and terminal settlement |
| Communication Note atomic 20-Point admission — source only | `20260902063211_add_v1_communication_note_points_admission.sql` and the TestOnly Points-admission adapter atomically admit one Communication durable job and reserve the fixed `2026-08-09.v1-shadow` rate of exactly 20 Points under an owner-wide advisory lock. The fifteenth forced-RLS private generation table binds job, quote and reservation one-to-one. Fresh success is `QUEUED`/attempt 0 with an `AVAILABLE` payload and one 20-Point `RESERVE`; exact replay is zero-write after revalidating session/privacy/expiry and binding state | default-off, Production-unapplied and unwired. `READY=false`; there is no pool, database URL lookup, caller grant or route importer. Migration #44 below replaces the temporary paid-job quarantine with exact reservation-gated worker and terminal rules, but creates no runtime principal or activation path. The DTO hides private binding and Points IDs, while historical owner RLS still permits the authenticated owner to read their own public Points row IDs |
| Communication Note atomic Points terminal settlement — source only | `20260902121601_add_v1_communication_note_points_terminal_settlement.sql` adds an isolated `NOLOGIN` settlement purpose role and two forced-RLS private tables. Success commits exactly 20 Points only in the transaction that proves the canonical document/revision, sync change, mutation receipt, provider evidence and purge outbox. Permanent failure or cancellation releases the exact source-lot allocation; retry retains `RESERVED`. Claim, recovery, heartbeat, authorize, fence, worker terminal and owner replay paths revalidate the immutable paid aggregate, including post-lock lease/fence clocks and worker-policy safety margins | default-off, Production-unapplied and unwired. No runtime role receives the settlement purpose role; API roles and generic Points commit/release remain blocked. Existing worker and owner envelopes stay byte-compatible and expose no binding, reservation or settlement identifier. Recovery alternates paid/unpaid and paid queued/running lanes per registration. The source and local-only gates do not constitute Hosted, deployment, activation, model-call, Production or real-care-data evidence |
...
- Assignment M1a originally passed 9 focused files / 292 tests and 138 files / 1,821 tests, TypeScript, lint, 64/64 static generation, 73-file adapter sync and diff checks. A PostgreSQL 16.15 minimum foundation→intake→detail→assignment chain passed the rollback suite plus true two-backend concurrency/session scenarios recorded in the Assignment checkpoint. Exact pre-review commit `526aa1e` later passed the deleted no-data Hosted 32/32 migration, then-current 13/13 rollback and real GoTrue SSR-cookie queue/detail/triage/candidates/offer/replay/revocation matrix. The 2026-08-26 queue-bound, shared-page-latch and null-adapter hardening passed 139 files / 1,830 tests and the revised Assignment rollback suite locally on PostgreSQL 16.15, then exact HEAD `43659ab16e9af6d9c73d0a55f8fe8b30b3ce9ee2` passed a separate Hosted gate on non-default, `persistent=false`, `with_data=false` branch `portal-assignment-m1a-r2-20260826` (id `3b111420-9f47-4e9f-b3a5-acd418f9423f`; ref `uzlnwjurzbtwtstabogm`). Supabase CLI 2.115.0 applied 32/32 migrations and all 13 current rollback suites completed. The exact-source local HTTPS Next runtime and real Hosted GoTrue/PostgREST path passed the eight-part SSR-cookie matrix: Bearer rejection; Source-A-only metadata queue; uniform Source-B/random-ID detail denial; triage plus exact replay; the one eligible provider candidate; offer plus exact replay; the expected OFFERED-v3/hash-only database state; and global-session revocation returning `401` from the same cookie. Teardown left Auth users, identities, sessions and refresh tokens plus every Portal fixture table at zero, all four Portal flags off and Preview-only, and the checked append-only triggers normally enabled. The branch was deleted and three consecutive list probes found both id and ref absent. Production retained 19 migrations and remained the default `ACTIVE_HEALTHY` project before and after; it received no SQL, Auth, route or other write. The page latch now enforces mutual exclusion on the shared `/referrals` URL while request-scoped Assignment API operation gates remain independent. At that M1a gate, offer kept `assigned_provider_id` null and provider accept/decline was absent; the following M1b bullet separates the historical and later exact-head Hosted evidence. This is exact-current Assignment disposable Hosted evidence, not a Vercel deployment, merge, retained activation or Production approval.
- Provider Response M1b initially passed 7 focused files / 244 tests and the full 142-file / 1,908-test suite. Historical exact source `cc1e53cc88666a3e3f18ac55058295db408535ee` then passed a separate no-data Hosted gate on deleted Preview ref `aupndcptwlqmjlgeifdj`. Post-review source `f45b19c596edd0bdbe01eba17e6e5fa136df5225` fixes accepted-history crowd-out, request/body hangs, repeated action names and cross-principal uncertain-command inheritance. It passed 8 focused files / 271 tests, 143 files / 1,935 tests, TypeScript, full lint, 64/64 static generation, 73-file adapter sync and diff checks; PostgreSQL 16.15 passed the 7/7 minimum migration chain, all four Portal rollback suites and 6/6 true two-backend replay/competition/revocation/lock-order scenarios with exact cleanup. Exact gate source HEAD `44f3bd68699dc953e2666bf033dac2b5e26a4d30` then passed a fresh disposable Hosted re-gate on `portal-provider-response-m1b-r2-20260826` (id `fb2e7d39-436d-48d5-a890-ad53b23b1fc6`; ref `nhupgyxczlvtddycrgyw`): 33/33 migrations, 14/14 rollback suites, historical Cookie/Data API assertions 14/14 and exact-current active-first/non-null-cursor checks 2/2. Teardown closed all five flags, removed all Auth/Portal fixtures, retained all three append-only triggers and zero API Portal table grants, then deleted the branch with three consecutive absence probes; Production remained the default `ACTIVE_HEALTHY` project with its exact 19 migrations. M1b remains default-off and Production-unapplied, with no retained Preview/runtime, Vercel deployment, merge or activation; private accepted detail, follow-up, notifications, audit listing and document/export remain outside the slice.
- The current PR's later UI-only recovery adds lost-focus restoration without focus theft plus an exact same-origin 401 sign-in return link. It passed 8 focused files / 274 tests and 143 files / 1,938 tests locally, but postdates `44f3bd6` and is not covered by that Hosted gate; no route, database, permission, deployment or activation changed.
- Read-only production logs showed a recent cluster of invalid/missing refresh-token errors. On 2026-09-04 the local source added a Next 16 request Proxy, request/response Cookie propagation, Supabase-provided anti-cache headers and a real `@supabase/ssr` test with a local fake Auth rejection that proves terminal stale-Cookie deletion. The full 234-file / 3,351-test suite and 64/64-page production build passed and the build identified the Proxy. This closes the source-level fix and reproducible local negative test only; a protected Vercel Preview with disposable real Supabase Auth and post-deployment observation remains required before V1 rollout.
...
- The production refresh-token error cluster now has a local Proxy/Cookie-cleanup mitigation and regression test, but no deployed Preview or live Production recovery evidence yet; the release gate remains open until that separate verification passes.
...
- The Communication-only source now couples encrypted staging and fixed
