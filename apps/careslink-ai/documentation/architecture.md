...
| Communication one-shot Preview-evaluation runner M1f | `communication-note-openai-request-template.ts`, `communication-note-preview-evaluation-manifest.ts` and `communication-note-preview-evaluation-runner.server.ts` literal-pin the static request semantics, exact six-slot order, no-retry worker policy and one-shot runner. The runner preflights every positive input count before dispatch, hashes each current rendered request, reserves a 120,780-micro-USD maximum below the US$0.25 cap, runs serially, reconciles strict provider usage, requires all seven checks plus 18 language reviews and returns a content-free, explicitly unattested self-digested report | source-only mock contract: paid readiness is `false`, approved runner policy is `undefined`, the paid factory fails closed and the executable factory accepts a branded test provider plus arbitrary trusted test callbacks that are not a security boundary. No independently approved exact wire-request pin, authenticated receipt, durable approval claim, API key, real reviewer attestation, OpenAI project/ZDR/AU/amendment proof, runtime importer, route, worker, Points, deployment or Production activation; zero real calls |
