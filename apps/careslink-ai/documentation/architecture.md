...
| Portal Referral intake runtime | `portal-referral-runtime.server.ts`, `portal-referral-supabase.server.ts`, the Portal route adapter, shared controls and migration `20260824124725_add_portal_referral_intake_runtime.sql` implement request-scoped cookie authorization, metadata-only source list and atomic create with database-derived actor/organization, private contact separation and replay receipts | default-off with source/local evidence plus the deleted r5 Hosted schema/transaction gate. Only `LIST_REFERRALS` and `CREATE_REFERRAL` are reachable when all three application gates, exact non-Production Preview ref and the separate database flag pass; every other Portal operation remains disabled. The database row remains `enabled=false, preview_only=true`; no retained Preview, hosted Auth/Data API E2E, Production migration, deployment, activation or business data is claimed |
