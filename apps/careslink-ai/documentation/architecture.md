...
- Assignment M1a adds a fourth default-off `referral_assignment_v1` row, six authenticated-only public RPCs and four private helpers for operator queue/detail, triage, candidates and offer. Its gate passed 9 focused files / 292 tests and 138 files / 1,821 tests, TypeScript, lint, 64/64 static generation, 73-file adapter sync and diff checks. A PostgreSQL 16.15 minimum foundation→intake→detail→assignment chain passed the rollback suite plus true two-backend concurrency/session scenarios recorded in the Assignment checkpoint. Exact commit `526aa1e` later passed the deleted no-data Hosted 32/32 migration, 13/13 rollback and real GoTrue SSR-cookie queue/detail/triage/candidates/offer/replay/revocation matrix. All temporary resources were removed. Offer keeps `assigned_provider_id` null; provider accept/decline and activation remain absent. The shared `/referrals` URL permits only one role surface to be activated at a time. This is exact Hosted Auth/Data API route evidence, not a Vercel Preview deployment, retained activation or Production evidence.
