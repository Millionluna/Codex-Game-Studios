...
- The local source also contains default-off Portal Referral intake and source-detail runtimes. They use a request-scoped cookie Supabase client and database authorization for source-scoped metadata list, atomic create and one exact-tenant private detail read. They are not part of the current Production deployment and have not been hosted or enabled on Preview.
...
| Portal Referral intake and source-detail runtime | `portal-referral-runtime.server.ts`, `portal-referral-supabase.server.ts`, the Portal route adapter, shared controls and migrations `20260824124725_add_portal_referral_intake_runtime.sql` and `20260825110251_add_portal_referral_source_detail_runtime.sql` implement request-scoped cookie authorization, metadata-only source list, atomic create and an exact-source-tenant private detail projection | default-off source/local implementation. `LIST_REFERRALS`/`CREATE_REFERRAL` and `GET_REFERRAL` have separate application and database operation gates. The master, intake and detail rows remain `enabled=false, preview_only=true`; direct Data API calls cannot use one operation's gate to open the other. Durable list items link to exact detail only when its gate is on; the route binds request/response IDs, projects the exact DTO, returns uniform not-found for absent/cross-tenant IDs and never falls back to mock. Triage, offer, response, follow-up and audit remain disabled. Deleted r5 evidence predates this exact migration, so no retained Preview, hosted Auth/Data API E2E, Production migration, deployment, activation or business data is claimed |
