...
`SUCCEEDED` and payload/outbox `PURGED` states. Catalog/registration rows must
remain append-only while attempts reference their digests or gain a reviewed
`RESTRICT` foreign key plus index. Nested database envelope exact-key vectors,
account-delete/purge recovery, provider `startedAt` binding to a consumed grant
with fresh lease/heartbeat, and safe sequential JSON numeric parsing remain
pre-activation governance. Payload vault/KMS/retention, caller/runtime wiring,
grants and activation also remain hard-off runtime blocks.
