...
`SKIP LOCKED` and session/privacy-revocation race gate. PostgreSQL 16 and owner
A/B runtime integration remain hard activation blockers. Deleted `r21` closed
the fixed Attempt-2-success historical replay gate across the later `RUNNING`,
...
`RESTRICT` foreign key and referencing-side index, but its fifteenth-migration
hosted Preview gate remains pending renewed cost confirmation. Nested database
envelope exact-key vectors, account-delete/purge recovery, provider `startedAt`
binding to a consumed grant with fresh lease/heartbeat, and safe sequential
JSON numeric parsing remain unproved pre-activation governance. Payload
vault/KMS/retention, caller/runtime wiring, grants and activation also remain
hard-off runtime blocks.
...
- Supabase CLI 2.115.0 generated the additive source-only `20260823213144_harden_v1_note_generation_registration_retention.sql` batch after the deleted `r21` gate. The migration creates `attempts_registration_digest_idx`, adds `attempts_registration_catalog_fk` from `attempts.registration_digest` to `worker_registrations.registration_digest` with update/delete `RESTRICT` and `NOT VALID`, then validates the constraint. It creates no seed, caller grant or runtime capability and has not been applied to Production. Because the historical branches proved only 14/14, this current fifteenth migration has no hosted Preview evidence; a new disposable run is expected to cover 15/15 only after renewed explicit cost confirmation.
- The current retention source gate passed 39/39 focused migration-contract tests, the full 125-file / 1,381-test suite, lint, TypeScript, the 63/63-page Next production build, the 73-file Codex-adapter sync check and `git diff --check`. This is local/static evidence only, not a hosted migration apply.
