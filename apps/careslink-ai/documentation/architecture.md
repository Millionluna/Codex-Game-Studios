...
| Communication activation preflight M1g-d | `communication-note-preview-activation-preflight.server.ts` binds version `preflight.communication.openai.synthetic-preview.2026-08-28.m1g-d.v1` and policy digest `81ab3c3bac64f2f9205c2eb358e298d440e3735e5a9c0ed07a842058e6947e53`. Its pure `TEST_ONLY` validator rebuilds a content-free candidate while cross-binding M1g-b authorization, M1g-c custody, an exact shared post-issuance observation time, six provider evidence values, a boundary-safe 36-migration manifest plus four artifact pins, one database project-reference HMAC domain claim, full unprivileged posture for four ordered 1/1/2/1 caller candidates, receipt lifecycle and an 18-review plan. All semantically distinct digests/HMACs/references are purpose-separated | source-only activation-preflight evidence, never activation authority. Valid input still yields five literal blockers and `activationReady=false`; the live factory always throws, all eight prior readiness latches remain false and all six approvals remain undefined. Database project HMACs/key reference and external evidence hashes/timestamps are injected consistency claims, not trusted control-plane/catalog/bytes attestations. No environment/network/SDK/resolver/database/runtime importer, migration, hosted action, paid call, deployment or Production change |
| Communication reserve-before-dispatch transcript M1g-e | `communication-note-preview-reserve-before-dispatch-coordinator.server.ts` binds version `coordinator.communication.openai.synthetic-preview.2026-08-28.m1g-e.v1` and policy digest `ea6bb5854783a322bd059abbf5c9f7d1e96828d2569e72bce8d23d4b196bf9b0`. Its pure `TEST_ONLY` validator reruns M1g-d, revalidates M1g-c custody and accepts only registration → exact six-slot runner preflight → parent-bound fresh claim with at least five minutes of authorization remaining → serial reservation → optional transport → signed receipt/persistence. `COMPLETED` also requires an explicit runner acceptance before continuation, bound to authorization/run/claim/reservation/receipt evidence, exact fixture/body/preflight/usage/cost, seven true critical checks and passed `en`/`zh-Hans`/`zh-Hant` reviews. Provider completion followed by runner failure is represented as a terminal no-retry candidate and cannot continue. `COMPLETED` and `PROVIDER_HTTP_ERROR` use a 30-second candidate interval; delayed ambiguity remains terminal and no-retry. Cross-role reuse across static evidence, client/transport/provider/candidate/receipt/fixture values is rejected, while runner provider hash versus receipt HMAC remains `UNATTESTED_NO_SHARED_IDENTIFIER` | source-only, default-off, content-free injected transcript validation, never an executable coordinator or database/provider attestation. Output is sanitized, recursively frozen and always reports `coordinatorReady=false`, `activationReady=false`, `dispatchCapability=ABSENT`, absent pre-run approval and absent post-run acceptance. The current reservation RPC does not return trusted `reservedAt`, and the current reserve RPC does not durably gate continuation on runner acceptance/failure, so `DATABASE_ATTESTED_RESERVED_AT_ABSENT` and `DURABLE_RUNNER_TERMINAL_STATE_ABSENT` remain fixed. Live factory always throws; no callback, environment/network/SDK/database/key operation, migration, hosted action, paid call, deployment or Production change |
