...
> Status: **Current-state architecture**, audited 2026-08-16; isolated database evidence updated 2026-08-25.
...
- The local source also contains default-off Portal Referral intake, source-detail and Assignment M1a runtimes. They use a request-scoped cookie Supabase client and database authorization for source-scoped intake/detail plus an operator-scoped assignment queue, private detail, triage, eligible-provider candidates and offer. Assignment remains separate from provider response and never sets `assigned_provider_id`. None of these Portal slices is part of the current Production deployment or enabled on Preview.
...
| Portal Referral intake, source-detail and Assignment M1a runtime | `portal-referral-runtime.server.ts`, `portal-referral-supabase.server.ts`, the Portal route adapter, dedicated assignment controls and migrations `20260824124725_add_portal_referral_intake_runtime.sql`, `20260825110251_add_portal_referral_source_detail_runtime.sql` and `20260825120908_add_portal_referral_assignment_runtime.sql` implement request-scoped cookie authorization, source list/create/detail and operator queue/detail/triage/candidates/offer | default-off source/local implementation. Intake, source detail and assignment have separate application and database operation gates under the master gate; direct Data API calls cannot use one operation's gate to open another. Assignment admits exactly one active platform-admin/PLATFORM or partner-operator/REFERRAL_SOURCE context, keeps tenant absence indistinguishable from not-found, derives eligible providers in one shared database authority, hashes mutation/correlation identifiers and returns strict metadata ACKs. Offer creates or promotes one match but keeps `assigned_provider_id=null`. Gate-on pages never fall back to mock. Provider accept/decline, follow-up, audit list, document/export and activation remain disabled. Deleted disposable pre-review commit `526aa1e` passed 32/32 migrations, the then-current 13/13 rollback suites and HTTPS SSR-cookie Hosted Auth/Data API E2E. The 2026-08-26 queue-bound, shared-page-latch and route-adapter hardening has local PostgreSQL 16.15 and application-suite evidence only and requires a new exact-current Preview for Hosted attribution. No retained Preview, Vercel deployment, Production migration, activation or business data exists |
...
- Assignment M1a originally passed 9 focused files / 292 tests and 138 files / 1,821 tests, TypeScript, lint, 64/64 static generation, 73-file adapter sync and diff checks. A PostgreSQL 16.15 minimum foundation→intake→detail→assignment chain passed the rollback suite plus true two-backend concurrency/session scenarios recorded in the Assignment checkpoint. Exact pre-review commit `526aa1e` later passed the deleted no-data Hosted 32/32 migration, then-current 13/13 rollback and real GoTrue SSR-cookie queue/detail/triage/candidates/offer/replay/revocation matrix. The 2026-08-26 queue-bound, shared-page-latch and null-adapter hardening instead passed 139 files / 1,830 tests and the revised Assignment rollback suite locally on PostgreSQL 16.15; its exact current SQL bodies have not been run on Hosted. The page latch now enforces mutual exclusion on the shared `/referrals` URL while request-scoped Assignment API operation gates remain independent. All temporary resources were removed. Offer keeps `assigned_provider_id` null; provider accept/decline and activation remain absent. A new exact-current disposable Preview is required to extend Hosted evidence to this hardening. This is historical Hosted plus current local evidence, not a Vercel Preview deployment, retained activation or Production evidence.
