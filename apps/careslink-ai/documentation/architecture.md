...
> Status: **Current-state architecture**, audited 2026-08-16; isolated database evidence updated 2026-08-21.
...
| Durable Note worker RPC shadow | CLI-generated `supabase/migrations/20260821071044_add_v1_note_generation_worker_rpc_shadow.sql` adds nine private metadata tables and the nine exact claim, heartbeat, fence, success, failure, resolve, recover, payload-authorize and payload-consume RPC identities. It binds fresh Auth/privacy reads, database time, worker/provider/payload registration digests, retry/recovery, canonical revision-1 persistence, provider evidence, payload logical revoke and purge-outbox acknowledgements | source-only, default-off and Production-unapplied. Catalogs and registrations are empty, the settings kill switch remains hard-off, all private tables use RLS plus FORCE RLS, RPCs are executor-owned `SECURITY DEFINER` functions with an empty `search_path`, and no API role or `service_role` has `EXECUTE`. With vault/KMS/retention undecided, consume can only settle `DENIED_SETTLED` / `PAYLOAD_UNAVAILABLE`; it returns no `vaultGrant`, locator or facts. The rollback SQL assertion's direct `TEST_ONLY` `CONSUMED` update exercises database atomicity only and is not payload-consume or vault E2E. Neither this migration nor its assertion has run on a database, Preview or Production |
...
diagram. Activation still requires PostgreSQL 16/17 disposable clean-apply,
both rollback assertions, exact role/ACL/`FOR SHARE` inspection, true
two-connection claim and authority-revocation races, injected-fault rollback
and a cross-domain zero-fixture result. Catalog/registration rows must remain
append-only while attempts reference their digests or gain a reviewed
`RESTRICT` foreign key plus index. Nested database envelope exact-key vectors,
attempt-2-success historical replay, account-delete/purge recovery, provider
`startedAt` binding to a consumed grant with fresh lease/heartbeat, and safe
sequential JSON numeric parsing remain pre-activation governance.
