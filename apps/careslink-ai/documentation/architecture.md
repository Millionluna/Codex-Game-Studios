...
| Communication resolved custody/caller runtime binding M1j | `communication-note-preview-runner-terminal-resolved-runtime-binding.server.ts` defines version `binding.communication.openai.synthetic-preview.2026-08-30.m1j.v1` and policy digest `6e33f6a6c061f539b75808afc27abfe6f33fedfcf28de5f7b2a5fdeed5faee04`. The TestOnly contract cross-binds an independently branded disposable-target descriptor, custody result, authorization ceiling and fifth-caller credential request; synchronously quarantines every recognizable lease reference, session binding, runtime role and module-local query identity before validating the remaining lease fields; uses PostgreSQL 17 `READ COMMITTED`, four separate transaction-local timeouts and `SET LOCAL ROLE`; attests stable backend PID/XID, database clock, exact caller/executor posture and role edges, schema/table/column/sequence ACL posture and exact terminal RPC metadata/ACL; bounds resolver/database/cleanup settlement with independent abort signals; and requires a strict digest-bound TestOnly release report after every acquisition attempt, including claims that its acquisition digest is tombstoned and future issuance is blocked | source-only/default-off architecture, not a live resolver. The public factory unconditionally fails without inspecting input; readiness is `false`; approved target, custody resolver, caller resolver and runtime port are all `undefined`; only the explicitly named TestOnly factory can execute. The TestOnly tombstone fields are resolver self-report, not durable broker evidence. No environment lookup, network/SDK client, raw credential/private key, product importer, migration, Preview, provider/model call, deployment or Production change was added. Real control-plane target provenance, approved resolver/session adapters with driver-level cancel/session destruction and durable acquisition-digest fencing, cross-process single-use enforcement and independent post-release role/session-zero verification remain activation blockers |
