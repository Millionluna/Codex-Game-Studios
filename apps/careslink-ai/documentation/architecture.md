...
| Communication signed runner-terminal caller and ports M1g-g | `communication-note-preview-runner-terminal-policy.server.ts` now binds an independent Ed25519 terminal trust root with purpose `CARESLINK_RUNNER_TERMINAL`, domain `CARESLINK_COMMUNICATION_NOTE_PREVIEW_RUNNER_TERMINAL`, policy version `policy.communication.openai.synthetic-preview.runner-terminal.2026-08-29.m1g-g.v2` and digest `d0ac3b14ceb97535cfed935250566b59d8ac42a93123a750d3a686102a8d1cfa`. Custody version `custody.communication.openai.synthetic-preview.2026-08-29.m1g-g.v2` and digest `f537dc64e3c57a34b6db6d0d1c871c38a70bcb51c4d071e625b026f840a309ca` add a purpose-separated terminal signer descriptor and fifth `RUNNER_TERMINAL_PERSISTENCE` mapping. Derived preflight/coordinator versions `preflight.communication.openai.synthetic-preview.2026-08-29.m1g-g.v3` and `coordinator.communication.openai.synthetic-preview.2026-08-29.m1g-g.v3` bind digests `491481513a67198cba91babc3c172fc1f326f9ee7bdd883b3d1208c639bdaf73` and `f6609c2f357b5fda92ae5aa1b459dfb1e32b7893c3e8436e0e94a8ffa2bbe675`. CLI migration `20260829011323_add_communication_note_preview_signed_terminal_caller_shadow.sql` replaces the unsigned two-argument terminal RPC with `persist_verified_communication_note_preview_runner_terminal(jsonb,text,text)`, persists the exact Base64URL signature and signer/verifier evidence, and adds `careslink_v1_preview_runner_terminal_caller` with only private-schema `USAGE` plus exact RPC `EXECUTE`. `communication-note-preview-signed-runner-terminal-runtime-port.server.ts` and `communication-note-preview-runner-terminal-postgres.server.ts` verify the signed envelope before an injected parameterized persistence call | source-only and default-off. The fifth caller is `NOLOGIN`/`NOINHERIT`/`NOBYPASSRLS`; it has no table, broad function, executor-membership, API/service-role or login privilege. The source-only runtime/Postgres ports cannot create a connection, read environment variables, resolve keys/credentials or choose another RPC; all readiness latches remain `false` and approved ports/signing key remain absent. The test-only signing-key snapshot is not yet cross-bound to a live M1g-c custody/trust-registry resolver, and the verifier HMAC is not cross-bound to a fifth-caller credential/identity resolver; both remain activation blockers. PostgreSQL stores application-verified evidence but does not claim Ed25519 verification. The local PostgreSQL 16.15 gate clean-applied 38/38 migrations; migration SHA-256 is `b095785331c848d02cabc417eb3131fe2f9328564abef6fc0dd35bccd2980c5a` and rollback-assertion SHA-256 is `f8e8307718e3bdf0835b93cdac075279ae4f5ba3dbab287af46e1280ce587ad5`. No Hosted action, provider/model call, deployment or Production change |
| Communication signed runner-terminal trust composition and disposable identity M1g-h | `communication-note-preview-runner-terminal-trust-composition.server.ts` brands a test-only registry and one custody/caller/signing-key composition in private `WeakMap` records, then cross-binds the signed verifier, runtime port and exact PostgreSQL port to that same composition without exposing raw key/caller inputs. The disposable-Preview harnesses add strict branch/Production/TLS checks, bounded stdin and fd3-only secret transport, a ten-minute one-connection `NOINHERIT` LOGIN design with only SET-only fifth-caller membership, fixed cleanup, an 18-file rollback manifest and a synthetic signed `FAILED` terminal chain | source and local PostgreSQL 16.15 gates passed, including exact 38 migrations and local signed `FAILED`/`CANCELLED` fresh/replay/conflict evidence, but the Hosted rollback runner returned `SCHEMA_ROLLBACK_ASSERTION_FAILED`; the Hosted identity and terminal gates were not run. The no-data Preview `hspkccjobyqmoomiidjp` was deleted and three listings left only healthy default Production. Registry/composition statuses remain `TEST_ONLY_*_NOT_APPROVED`, readiness is `false`, approved values are `undefined`, and `ACCEPTED` remains blocked by a 9-key source versus 6-key receipt-usage mismatch. No provider/model call, real care data, deployment, Production write, push, merge or activation occurred |
| Communication signed terminal ACCEPTED usage alignment M1g-i | Additive migration `20260829041316_align_communication_note_preview_terminal_accepted_usage.sql` replaces only the existing signed three-argument terminal RPC: it validates and persists the exact nine-key signed usage statement with deterministic `C` collation, projects away only the three reconciliation labels for an exact comparison with the receipt ledger's six provider facts, and preserves signature, cost, lock, append-only, replay and ACL semantics. The rollback harness emits fixed content-free `R00`/`A01`–`A18` stages; preflight/coordinator versions are `m1g-i.v4` with digests `59a71d4e8668f16cbb0007cfa13bca595b4767cc4842a0d0bb69be0708e9a4ae` and `02268069d8290059988bf96d488828b0b12dec912810972b97c790629fa848af` | source/local PostgreSQL 16.15 passed 39/39, 18/18 and positive/negative vectors. Deleted no-data Hosted PostgreSQL 17 r20 then passed 39/39, A01–A18 and a real temporary-LOGIN signed `ACCEPTED`/replay/conflict E2E; independent postcheck found `[1,0,1,1,1,1]` ledgers and zero temporary LOGINs/sessions. Three post-delete listings left only healthy Production. Migration 38 remains historical and migration 39 adds no table, role, credential or product surface. Live custody/credential resolvers, provider/model evaluation, human review and final approval remain absent; readiness is `false`, approved values are `undefined`, and no model, real-data, deployment or Production write occurred |
