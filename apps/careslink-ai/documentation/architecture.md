...
| Durable Note metadata schema foundation | `supabase/migrations/20260820135834_add_v1_note_generation_durable_shadow.sql` creates dedicated non-login owner/executor roles, one private schema, a forced-off settings row, metadata-only jobs/attempts and supporting indexes; all three tables enable and force RLS, define no policy and expose no API/executor privilege | generated by Supabase CLI 2.115.0 and Production-unapplied. A fresh `with_data=false` PostgreSQL 17 `r3` branch clean-applied all 13 exact source migrations; the repaired `pg_constraint` rollback assertion, post-rollback ACL/RLS/role/zero-fixture checks and generation-schema advisor review passed. Four adjacent rollback suites also passed, but the Portal Referral suite hit `VALIDATION_ERROR` in an older canonical-revision fixture that lacks the current privacy proof/facts binding and rolled back. The local privacy-bound fixture repair now exists and passes its source contract; the deleted `r3` therefore proves this isolated foundation, while the complete exact-revision cross-domain gate still requires a fresh `r4` database run. No function, view, trigger, RPC, payload table/grant, purge outbox, operational timing/retention value or runtime registration exists; Production was not the SQL target, and no deployment, runtime flag or API/executor grant was added or enabled |
...
| Mobile sync schema/RPC draft | `supabase/migrations/20260810131648_add_v1_mobile_sync_shadow.sql` plus `supabase/tests/v1_mobile_sync_shadow_assertions.sql` add service-only session status, session-checked owner reads, document mutations and the change feed | Production-unapplied and clean-applied only on deleted disposable Previews; only list/get/pull are drafted for authenticated execution and write-RPC grants remain withheld. The exact SQL assertion passed on deleted `r3`, but the complete exact-revision cross-domain gate remains incomplete because the later Portal Referral assertion fixture failed against the current privacy-proof contract |
