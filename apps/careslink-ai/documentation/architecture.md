...
| Portal Referral Follow-up M1c runtime | `portal-referral-runtime.server.ts`, `portal-referral-supabase.server.ts`, the dedicated provider follow-up coordinator and CLI-generated `20260826090841_add_portal_referral_follow_up_runtime.sql` implement exact accepted-provider private detail plus fixed-code follow-up | independently default-off and Production-unapplied. Three authenticated-only definer RPCs re-derive exact approved-provider authority, bind detail to assigned provider plus coherent accepted match, and atomically write referral version, append-only follow-up, metadata-only audit and hash-only receipt. The client clears private state on identity/resource changes and preserves no PII in uncertain replay state. Source/local tests and a retained fresh-cluster PostgreSQL 16.15 lifecycle now prove 8/8 true multi-backend replay/competition/revocation/ownership races with exact cleanup. No Hosted GoTrue/PostgREST, retained Preview, deployment, activation or Production application exists |
...
- Follow-up M1c source/local validation first passed 11 focused files / 336 tests, the full 147-file / 2,011-test suite, TypeScript, lint and the 64/64-page production build. Independent state-machine review found no remaining P0/P1 after resource/authorization-epoch fencing, per-resource write registration and authoritative stale-completion reconciliation. Its later retained concurrency lifecycle passed 2 focused files / 23 tests, the current 149-file / 2,034-test suite, TypeScript and lint, then clean-applied the exact eight-file chain on fresh PostgreSQL 16.15 and passed 8/8 true multi-backend replay/competition/session/provider/flag/ownership scenarios. The runner was made `NOLOGIN`; SQL cleanup and independent postcheck retained six disabled Preview-only flags, zero Auth/Portal fixtures and all three append-only triggers; PostgreSQL stopped normally and the exact temporary root was deleted. This remains local engine/schema/transaction evidence only: Hosted Auth/Data API, Vercel deployment, activation, merge and Production application are still absent.
