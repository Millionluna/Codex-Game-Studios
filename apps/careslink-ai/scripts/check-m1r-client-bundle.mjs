...
if (!existsSync(buildIdPath) || !existsSync(chunksRoot)) {
  throw new Error("M1r/M1s/M1t/M1u/M1v/M2a/M2b client-boundary check requires a completed Next.js build");
}
...
if (chunkFiles.length === 0) {
  throw new Error("M1r/M1s/M1t/M1u/M1v/M2a/M2b client-boundary check found no static chunk files");
}
for (const file of chunkFiles) {
  const source = readFileSync(file, "utf8");
  const marker = forbiddenMarkers.find((candidate) => source.includes(candidate));
  if (marker) {
      throw new Error(`M1r/M1s/M1t/M1u/M1v/M2a/M2b server-only marker leaked into client chunk: ${marker}`);
  }
}

process.stdout.write(
  `M1r/M1s/M1t/M1u/M1v/M2a/M2b client boundary passed across ${chunkFiles.length} static chunk files.\n`,
);
