...
export async function installTaskCredentialBroker(root, open, capability) {
  ...
  await owner.query(`create schema cl_local_task_credential;
    ...
    create table ${TABLE}(id text primary key, role_name text unique not null, role_oid oid not null,
      ...
      revoked_at timestamptz, reason text check(reason in ('COMPLETE','EXPIRED','STOP')));
  ...
  const server = createServer({ requestTimeout: 3000, headersTimeout: 2000, maxHeaderSize: 1024 }, (req, res) => {
  ...
  return Object.freeze({
    ...
    async stop() {
      stopped = true; const closed = new Promise(resolve => server.close(resolve));
