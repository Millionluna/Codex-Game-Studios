...
export const COMMUNICATION_NOTE_POINTS_ADMISSION_DATABASE_URL_ENV =
  "CARESLINK_V1_COMMUNICATION_NOTE_POINTS_ADMISSION_LOCAL_PG16_DATABASE_URL";
export const COMMUNICATION_NOTE_POINTS_ADMISSION_RUNNER_ROLE =
  "careslink_v1_communication_admission_concurrency_runner";
export const COMMUNICATION_NOTE_POINTS_ADMISSION_SUPPORT_SCHEMA =
  "careslink_v1_communication_admission_concurrency_test_support";
export const COMMUNICATION_NOTE_POINTS_ADMISSION_MARKER =
  "2026-09-02.local-pg16.communication-admission.1";

export const COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY = Object.freeze({
  version: COMMUNICATION_NOTE_POINTS_ADMISSION_MARKER,
  requiredScheme: "postgresql:",
  connectionUriHost: "localhost",
  requiredDatabase: "postgres",
  requiredRole: COMMUNICATION_NOTE_POINTS_ADMISSION_RUNNER_ROLE,
...
export const COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES =
  Object.freeze({
    invalidInput:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_POLICY_INVALID",
    urlMissing:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_DATABASE_URL_MISSING",
    urlInvalid:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_DATABASE_URL_INVALID",
    targetDenied:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_TARGET_DENIED",
    roleDenied:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_ROLE_DENIED",
    credentialsDenied:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_CREDENTIALS_DENIED",
    databaseDenied:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_DATABASE_DENIED",
    portDenied:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_PORT_DENIED",
    queryDenied:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_QUERY_DENIED",
    environmentDenied:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_ENVIRONMENT_DENIED",
    preflightFailed:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_PREFLIGHT_FAILED",
    backendIdentityFailed:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_BACKEND_IDENTITY_FAILED",
    blockerFailed:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_LOCK_NOT_OBSERVED",
    sqlPolicyInvalid:
      "COMMUNICATION_NOTE_POINTS_ADMISSION_CONCURRENCY_SQL_POLICY_INVALID",
  });
...
export class CommunicationNotePointsAdmissionConcurrencyPolicyError extends Error {
  constructor(code) {
    const fixedCode = POLICY_ERROR_CODE_SET.has(code)
      ? code
      : COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.invalidInput;
    super(fixedCode);
    this.name = "CommunicationNotePointsAdmissionConcurrencyPolicyError";
    this.code = fixedCode;
  }
}

function fail(code) {
  throw new CommunicationNotePointsAdmissionConcurrencyPolicyError(code);
}
...
function parseUrl(value) {
  if (
    typeof value !== "string" ||
    value.length === 0 ||
    value.length > COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY.maximumUrlLength ||
    value !== value.trim() ||
    CONTROL_CHARACTER_PATTERN.test(value)
  ) {
    fail(COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.urlInvalid);
  }
 ...
 */
export function validateCommunicationNotePointsAdmissionDatabaseUrl(value) {
  const policy = COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY;
  ...
  if (
    ...
    fail(COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.targetDenied);
  }
  if (url.username !== policy.requiredRole) {
    fail(COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.roleDenied);
  }
  if (url.password.length !== 0) {
    fail(
      COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.credentialsDenied,
    );
  }
  if (url.pathname !== `/${policy.requiredDatabase}`) {
    fail(COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.databaseDenied);
  }
  if (value.includes("#")) {
    fail(COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.queryDenied);
  }
  ...
  if (
    ...
    fail(COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.portDenied);
  }
...
export function readCommunicationNotePointsAdmissionEnvironment(env) {
  if (env === null || typeof env !== "object") {
    fail(
      COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.environmentDenied,
    );
  }

  for (const name of COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY
  ...
  const databaseUrl =
    env[COMMUNICATION_NOTE_POINTS_ADMISSION_DATABASE_URL_ENV];
  if (typeof databaseUrl !== "string" || databaseUrl.length === 0) {
    fail(COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.urlMissing);
  }

  return Object.freeze({
    databaseUrl,
    target: validateCommunicationNotePointsAdmissionDatabaseUrl(databaseUrl),
  });
}

export function assertCommunicationNotePointsAdmissionPreflight(row, target) {
  const policy = COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY;
  if (
    row === null ||
    typeof row !== "object" ||
    ...
    fail(COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.preflightFailed);
  }
...
export function assertCommunicationNotePointsAdmissionDistinctBackends(...pids) {
  if (
    pids.length < 2 ||
    pids.some(
...
export function assertCommunicationNotePointsAdmissionBlockerRows(
  rows,
  waiterPid,
  blockerPid,
) {
...
export function assertCommunicationNotePointsAdmissionSqlPolicy(
  setupSql,
  cleanupSql,
) {
  if (typeof setupSql !== "string" || typeof cleanupSql !== "string") {
    fail(COMMUNICATION_NOTE_POINTS_ADMISSION_POLICY_ERROR_CODES.sqlPolicyInvalid);
  }

  const requiredSetup = [
    "COMMUNICATION_ADMISSION_CONCURRENCY_SETUP_UNSAFE",
    "COMMUNICATION_ADMISSION_CONCURRENCY_SCHEMA_DRIFT",
    "COMMUNICATION_ADMISSION_CONCURRENCY_DATABASE_NOT_EMPTY",
    "careslink_v1_communication_admission_concurrency_runner",
    "careslink_v1_communication_admission_concurrency_test_support",
  ...
  const requiredCleanup = [
    "COMMUNICATION_ADMISSION_CONCURRENCY_CLEANUP_UNSAFE",
  ...
  const markerGuardDisableCount =
    cleanupSql.match(
      /\bdisable\s+trigger\s+jobs_communication_note_point_marker_guard\b/gi,
    )?.length ?? 0;
  const markerGuardEnableCount =
    cleanupSql.match(
      /\benable\s+trigger\s+jobs_communication_note_point_marker_guard\b/gi,
    )?.length ?? 0;
  ...
  const exactSyntheticUnbindOrder =
    /disable\s+trigger\s+jobs_communication_note_point_marker_guard\s*;[\s\S]*?update\s+careslink_v1_generation\.jobs\s+set\s+communication_note_point_admission_id\s*=\s*null\s+where\s+owner_user_id\s+in\s*\([\s\S]*?\)\s+and\s+communication_note_point_admission_id\s+is\s+not\s+null\s*;[\s\S]*?enable\s+trigger\s+jobs_communication_note_point_marker_guard\s*;[\s\S]*?disable\s+trigger\s+communication_note_point_admissions_immutable\s*;/i;
  ...
  const bootstrapRoleParameter =
    /\\if\s+:\{\?careslink_bootstrap_role\}[\s\S]{0,220}?pg_catalog\.set_config\(\s*'careslink\.communication_admission_concurrency_bootstrap_role',\s*:'careslink_bootstrap_role',\s*true\s*\)/i;
  ...
  const capturedBootstrapIdentity =
    /create function[\s\S]{0,180}?\._assert_runner\(\)[\s\S]{0,240}?security definer[\s\S]{0,120}?set search_path = ''[\s\S]{0,180}?set careslink\.communication_admission_concurrency_bootstrap_role from current/i;
  ...
  if (
    ...
    !/create role careslink_v1_communication_admission_concurrency_runner\s+[\s\S]{0,240}?login[\s\S]{0,240}?noinherit[\s\S]{0,240}?nobypassrls/i.test(
      setupSql,
    ) ||
    !/create role careslink_v1_communication_admission_concurrency_runner\s+[\s\S]{0,320}?password null[\s\S]{0,80}?connection limit 3/i.test(
      setupSql,
    ) ||
    !/grant execute on function\s+careslink_v1_generation\.admit_and_reserve_v1_shadow_communication_note_generation_job\s*\(/i.test(
      setupSql,
    ) ||
    /grant\s+(?:all|select|insert|update|delete)[\s\S]{0,160}?to careslink_v1_communication_admission_concurrency_runner/i.test(
      setupSql,
    ) ||
